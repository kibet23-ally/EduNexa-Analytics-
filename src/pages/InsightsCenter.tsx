      ...
      didParseCell: (data) => {
        if (data.section === 'body') {
          if (data.column.index === 6) {
            const val = parseFloat(String(data.cell.text[0]));
            if (!isNaN(val)) data.cell.styles.textColor = val >= 0 ? [66, 177, 41] as any : [220, 38, 38] as any;
          }
          if (data.column.index === 7) {
            const t = String(data.cell.text[0]);
            if (t.includes('▲')) data.cell.styles.textColor = [66, 177, 41] as any;
...
async function generateReportCard(params: {
  ...
  if (subjectMarks.length === 0) {
    ...
    if (subjectMarks.length > 0) {
      ...
      doc.setFillColor(18, 52, 101, 0.08 as any);
      const areaPath = [
    ...
    if (prevSubjectMarks && prevSubjectMarks.length > 0) {
      ...
      doc.setFillColor(18, 52, 101); doc.circle(M + 43.5, legendY, 1, 'F');
    ...
    /* ── Authenticity notice ── */
    doc.setFont('helvetica', 'italic'); doc.setFontSize(6.5); doc.setTextColor(30, 58, 95);
    doc.text(
