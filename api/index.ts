 ...
 *
 * IMPORTANT: this file is intentionally self-contained (no imports
 * reaching outside /api) rather than importing the shared logic from
 * ../src/apiApp.ts. Vercel's ESM function bundling for a "type": "module"
 * function failed to trace and include that cross-directory import at
 * runtime (ERR_MODULE_NOT_FOUND for /var/task/src/apiApp), even though
 * the same relative import works fine for the traditional persistent
 * server (server.ts) locally. Keeping everything in one file inside the
 * directory Vercel actually scans sidesteps that bundling edge case.
 *
 * src/apiApp.ts remains the source of truth for the traditional server
 * (server.ts, used for local dev and any non-Vercel host). If you change
 * a route here, change it there too -- see server.ts's own comment.
 */
...
/**
 * Builds the Express app containing every /api/* route plus the shared
 * middleware (compression, JSON body parsing, subscription gating, the
 * global error-handling backstop). Used by BOTH:
 *  - server.ts, the traditional persistent Node server (local dev, or any
 *    non-Vercel host that supports a long-running process), and
 *  - api/[...slug].ts, the Vercel serverless function entrypoint.
 * Keeping the routes in one place means those two never drift into
 * independently-maintained copies of the same API.
 */
const app = express();
  ...
  // gets dropped with an empty body.
