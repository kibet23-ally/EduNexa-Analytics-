...
const SchoolDashboard: React.FC = () => {
  ...
  /* ── Attendance — key includes today so cache busts each new day ── */
  const attendanceQuery = useData<any>(
    `dashboard-attendance-${today}`,          // ← date in key = auto cache bust
    'attendance_daily',
    {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/40 to-cyan-50/30 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 p-4 md:p-6 lg:p-8">
      ...
      {/* ════════════ ROW 1 — Enrollment + Today's Attendance ════════════ */}
      ...
      {/* ════════════ ROW 2 — Academic Performance + Attendance Breakdown by Grade ════════════ */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8">
