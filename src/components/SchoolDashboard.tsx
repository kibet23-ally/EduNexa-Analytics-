...
const SchoolDashboard = () => {
  ...
  // Gate ALL queries on sessionReady AND school_id
  // Without sessionReady, queries fire before JWT is set → RLS returns 0
  ...
  const isLoading = !sessionReady ||
    studentCount.isLoading || gradeCount.isLoading ||
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {isLoading ? (
          ...
          cards.map((card, i) => (
            <div key={i} className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex items-center gap-4 group hover:shadow-lg transition-all cursor-default">
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden relative">
        <div className="relative z-10">
          ...
          {attendance.isLoading || !sessionReady ? (
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-8">
              {[
                ...
                { label: 'Excused',        value: attendanceSummary.excused,  bg: 'bg-blue-50 dark:bg-blue-900/20 border-blue-100 dark:border-blue-900/30',     text: 'text-blue-700 dark:text-blue-400',           label_color: 'text-blue-600 dark:text-blue-400', hidden: true },
              ].map((item) => (
                <div key={item.label} className={`text-center p-6 rounded-2xl border ${item.bg} ${item.hidden ? 'hidden lg:block' : ''}`}>
                  <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${item.label_color}`}>{item.label}</p>
                  <p className={`text-3xl font-display font-bold ${item.text}`}>{item.value}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={performanceData}>
                ...
                <Tooltip
                  cursor={{ fill: '#f8fafc' }}
                  contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)', padding: '12px', backgroundColor: '#1e293b', color: '#fff' }}
                />
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800">
          <h3 className="text-xl font-display font-bold text-slate-900 dark:text-white mb-8 flex items-center gap-3">
            <Award size={24} className="text-accent" /> Quick Actions
          </h3>
          <div className="space-y-4">
            <button onClick={() => navigate('/marks')}
              className="w-full text-left px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800 hover:bg-primary/5 dark:hover:bg-primary/10 text-slate-700 dark:text-slate-300 hover:text-primary transition-all text-sm font-bold border border-transparent hover:border-primary/10">
              Enter Student Marks
            </button>
