        ...
        >
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-xl text-sm
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                type="date" value={form.event_date} onChange={set('event_date')} required
                className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-xl text-sm
                  bg-white dark:bg-slate-800 text-slate-800 dark:text-white
                  focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
            ...
            <div>
              ...
              <input
                type="time" value={form.event_time} onChange={set('event_time')}
                className="w-full px-3 py-2 border border-slate-200 dark:border-white/10 rounded-xl text-sm
                  bg-white dark:bg-slate-800 text-slate-800 dark:text-white
                  focus:outline-none focus:ring-2 focus:ring-blue-500"
              />
          ...
          {saveError && (
            <p className="text-xs text-rose-600 bg-rose-50 dark:bg-rose-500/10 px-3 py-2 rounded-lg">
              {saveError}
            </p>
          )}
          <div className="flex gap-3 pt-1">
            <button
              ...
              className="flex-1 px-4 py-2 border border-slate-200 dark:border-white/10
            ...
            <button
              ...
              className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-xl text-sm font-medium
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm p-6 border border-slate-200 dark:border-white/10">
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          className="flex-1 px-4 py-2 bg-rose-600 text-white rounded-xl text-sm font-medium
            hover:bg-rose-700 disabled:opacity-60 transition-colors"
        >
...
/* ─────────────────────────────────────────────────────────────
   ...
   Uses exams columns: id, exam_name, term (int), year (int)
───────────────────────────────────────────────────────────── */
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const row of rows ?? []) {
        const exam = (row as any).exams as {
          id: number; exam_name: string; term: number; year: number;
        } | null;
...
function useSubjectPerformance(schoolId: number | undefined) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const row of rows ?? []) {
        const subject = (row as any).subjects as {
...
const SchoolDashboard: React.FC = () => {
  ...
  const examsQuery = useData<any>(
    ...
    { filters: { school_id: user?.school_id }, orderBy: { column: 'year', ascending: false } },
    enabled,
  );

  /* ── Attendance — uses attendance_daily view to avoid per-subject inflation ── */
  const today           = new Date().toISOString().slice(0, 10);
  const attendanceQuery = useData<any>(
    'dashboard-attendance', 'attendance_daily',
    { select: 'student_id, status, date', filters: { school_id: user?.school_id, date: today } },
    enabled,
  );
  ...
  /* ── Events hook ── */
  const {
    upcoming, loading: loadingEvents, error: errorEvents,
    add, update, remove,
  ...
  /* ── Derived attendance counts (from daily view — 1 row per student) ── */
  const attendanceRows = attendanceQuery.data ?? [];
  const present        = attendanceRows.filter((a: any) => a.status?.toLowerCase() === 'present').length;
  const absent         = attendanceRows.filter((a: any) => a.status?.toLowerCase() === 'absent').length;
  const late           = attendanceRows.filter((a: any) => a.status?.toLowerCase() === 'late').length;
  ...
  /* ── Enrollment trend (derived from live student count) ── */
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/40 to-cyan-50/30 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900 p-4 md:p-6 lg:p-8">
      ...
      {showAdd && (
        <EventModal
          mode="add" initial={EMPTY_FORM} onSave={handleAdd}
      ...
      {/* ════════════ ROW 1 — Enrollment + Attendance Radial ════════════ */}
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8">
        ...
        <GlassCard className="lg:col-span-2 p-6">
          ...
          {loadingPerf ? (
            ...
            <div className="h-72 flex items-center justify-center text-sm text-rose-500">
              {errorPerf}
            </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Pie
                  ...
                  dataKey="value" innerRadius={55} outerRadius={85}
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={subjectData} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(148,163,184,0.2)" vertical={false} />
                  <XAxis
                    dataKey="subject" tickLine={false} axisLine={false}
                    tick={{ fill: '#94a3b8', fontSize: 12 }}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500 flex items-center gap-1 mt-0.5">
                        <Clock size={11} />
                        {new Date(ev.event_date + 'T00:00:00').toLocaleDateString('en-KE', {
