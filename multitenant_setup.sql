...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='teacher_subjects' AND column_name='school_id') THEN
    ALTER TABLE teacher_subjects ADD COLUMN school_id BIGINT REFERENCES schools(id);
  END IF;
END $$;
...
UPDATE teacher_subjects SET school_id = (SELECT id FROM schools WHERE slug = 'default-school') WHERE school_id IS NULL;
...
ALTER TABLE teachers ADD CONSTRAINT teachers_role_check CHECK (role IN ('Teacher', 'Admin', 'SuperAdmin'));
