...
import { PDF_COLORS, PDF_TABLE_THEME, addBorderToAllPages } from '../lib/pdfKit';
...
function drawPDFLetterhead(
  ...
  // Header rule (monochrome — no fills)
  doc.setDrawColor(...PDF_COLORS.black); doc.setLineWidth(0.6);
  doc.line(M, 33, W - M, 33);

  // Logo — left. Top kept clear of the page border (drawn at y=10 by
  // addBorderToAllPages), which a logo flush against y=4 used to collide with.
  if (logo) {
    try { doc.addImage(logo.data, logo.fmt, M, 13, 18, 18); } catch { /* noop */ }
  }
  // No logo = no placeholder box, school name stays centered

  // Photo placeholder (portrait only, report card) — same top-clearance fix.
  if (showPhoto) {
    doc.setDrawColor(...PDF_COLORS.medGray); doc.setLineWidth(0.3);
    doc.rect(W - M - 18, 13, 18, 18, 'S');
    doc.setTextColor(...PDF_COLORS.medGray); doc.setFontSize(6); doc.setFont('helvetica', 'normal');
    doc.text('PHOTO', W - M - 9, 23, { align: 'center' });
  }

  // School name — y raised from 12 to 18 so its ascender clears the border.
  doc.setTextColor(...PDF_COLORS.black);
  doc.setFontSize(16); doc.setFont('helvetica', 'bold');
  doc.text((school?.name || 'SCHOOL').toUpperCase(), W / 2, 18, { align: 'center' });

  // Contacts
  const contacts = [school?.address, school?.phone, school?.email].filter(Boolean).join('  |  ');
  if (contacts) {
    doc.setFontSize(7); doc.setFont('helvetica', 'normal'); doc.setTextColor(...PDF_COLORS.darkGray);
    doc.text(contacts, W / 2, 24, { align: 'center' });
  }

  // Motto
  if (school?.motto) {
    doc.setFontSize(8); doc.setFont('helvetica', 'italic'); doc.setTextColor(...PDF_COLORS.darkGray);
    doc.text(`"${school.motto}"`, W / 2, 29, { align: 'center' });
  }

  // Title ribbon
  doc.setDrawColor(...PDF_COLORS.black); doc.setLineWidth(0.5);
  doc.rect(M, 37, W - M * 2, 9, 'S');
  doc.setTextColor(...PDF_COLORS.black); doc.setFontSize(10); doc.setFont('helvetica', 'bold');
...
function drawPDFFooter(doc: jsPDF, school: School | undefined) {
  ...
  // Pulled up from H-10/H-5 (which sat exactly on, and past, the bottom
  // border) so the rule and text both stay clearly inside it.
  doc.setDrawColor(...PDF_COLORS.black); doc.setLineWidth(0.3);
  doc.line(14, H - 16, W - 14, H - 16);
  doc.setFont('helvetica', 'normal'); doc.setFontSize(6.5); doc.setTextColor(...PDF_COLORS.medGray);
  doc.text(`${school?.name || ''} • Generated by EduNexa Analytics`, 14, H - 12);
  doc.text(new Date().toLocaleDateString('en-KE', { dateStyle: 'full' }), W - 14, H - 12, { align: 'right' });
}
...
async function generateRankingsPDF(params: {
  ...
  doc.setDrawColor(54, 54, 54); doc.setLineWidth(0.2);
  doc.rect(M, 49, W - M * 2, 10, 'S');
  doc.setFont('helvetica', 'bold'); doc.setFontSize(8); doc.setTextColor(54, 54, 54);
  ...
  doc.setFont('helvetica', 'bold'); doc.setFontSize(9); doc.setTextColor(54, 54, 54);
  ...
  autoTable(doc, {
    ...
    headStyles: { fillColor: [0, 0, 0], textColor: [255, 255, 255], fontStyle: 'bold', fontSize: 7.5, halign: 'center' },
    ...
    didParseCell: (data) => {
      if (data.section === 'body' && data.column.index === 5) {
        ...
        if (r) { data.cell.styles.textColor = [0, 0, 0]; data.cell.styles.fontStyle = 'bold'; }
      }
    },
  });
    ...
    didParseCell: (data) => {
      if (data.section === 'body') {
        ...
        if (data.column.index === plCol) {
          const code = String(data.cell.text[0]);
          const r = RUBRIC.find(r => r.code === code);
          if (r) data.cell.styles.textColor = [0, 0, 0];
        }
  ...
  doc.setTextColor(215, 215, 215); doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5);
  doc.text(
  ...
  if (prevMarks && prevMarks.length > 0 && prevExamName) {
    doc.setFont('helvetica', 'normal'); doc.setFontSize(8); doc.setTextColor(54, 54, 54);
      ...
      didParseCell: (data) => {
        if (data.section === 'body') {
          if (data.column.index === 6) {
            const val = parseFloat(String(data.cell.text[0]));
            if (!isNaN(val)) { data.cell.styles.textColor = [0, 0, 0]; data.cell.styles.fontStyle = 'bold'; }
          }
          if (data.column.index === 7) {
            const t = String(data.cell.text[0]);
            if (t.includes('▲') || t.includes('▼')) data.cell.styles.fontStyle = 'bold';
    ...
    doc.setDrawColor(54, 54, 54); doc.setLineWidth(0.3);
    ...
    doc.setFont('helvetica', 'italic'); doc.setFontSize(10); doc.setTextColor(114, 114, 114);
...
async function generateReportCard(params: {
  ...
  doc.setDrawColor(171, 171, 171); doc.setLineWidth(0.2);
  doc.rect(M, panelY, W - M * 2, 26, 'S');
  doc.setTextColor(215, 215, 215); doc.setFontSize(7.5); doc.setFont('helvetica', 'bold');
  ...
  if (subjectMarks.length === 0) {
    doc.setTextColor(129, 129, 129); doc.setFontSize(9); doc.setFont('helvetica', 'italic');
    ...
    autoTable(doc, {
      ...
      theme: 'grid',
      styles: { fontSize: 7.5, cellPadding: 2.2, lineColor: [0, 0, 0], lineWidth: 0.2 },
      headStyles: {
        fillColor: [0, 0, 0], textColor: [255, 255, 255], fontStyle: 'bold',
        fontSize: 7.5, halign: 'center', lineColor: [0, 0, 0], lineWidth: 0.3,
      },
      alternateRowStyles: { fillColor: [245, 245, 245] },
      columnStyles: {
        0: { cellWidth: 52 },
      ...
      didParseCell: (data) => {
        if (data.section === 'body') {
          if (data.column.index === 1) {
            const val = parseFloat(String(data.cell.text[0]));
            if (!isNaN(val)) data.cell.styles.fontStyle = 'bold';
          }
    ...
    doc.setFontSize(5); doc.setFont('helvetica', 'normal'); doc.setTextColor(149, 149, 149);
    ...
    if (subjectMarks.length > 0) {
      ...
      doc.setFillColor(0, 0, 0, 0.06 as any);
      const areaPath = [
      ...
      doc.setFillColor(223, 223, 223);
      doc.setDrawColor(223, 223, 223);

      doc.setDrawColor(47, 47, 47); doc.setLineWidth(0.8);
      ...
      pts.forEach(p => {
        ...
        doc.setFont('helvetica', 'normal'); doc.setFontSize(4.8); doc.setTextColor(89, 89, 89);
    ...
    if (prevSubjectMarks && prevSubjectMarks.length > 0) {
      ...
      doc.setFillColor(47, 47, 47); doc.circle(M + 43.5, legendY, 1, 'F');
    ...
    doc.setDrawColor(47, 47, 47); doc.setLineWidth(0.2);
    doc.rect(M, attY, W - M * 2, 8, 'S');
    doc.setFont('helvetica', 'bold'); doc.setFontSize(7.5); doc.setTextColor(54, 54, 54);
    doc.text(
    ...
    doc.setDrawColor(171, 171, 171); doc.setLineWidth(0.3);
    doc.rect(M, remY, halfW, 32, 'FD');
    doc.setFillColor(54, 54, 54);
    doc.rect(M, remY, halfW, 6, 'F');
    doc.setTextColor(215, 215, 215); doc.setFont('helvetica', 'bold'); doc.setFontSize(8);
    doc.text("CLASS TEACHER'S REMARKS", M + 3, remY + 4.5);
    doc.setTextColor(49, 49, 49); doc.setFont('helvetica', 'italic'); doc.setFontSize(7.5);
    ...
    /* ── Authenticity notice ── */
    doc.setFont('helvetica', 'italic'); doc.setFontSize(6.5); doc.setTextColor(54, 54, 54);
    doc.text(
            ...
            <div className="flex flex-wrap items-center gap-2">
              {[
                ...
                <select key={f.ph} value={f.val} onChange={e => f.set(e.target.value)}
                  className="text-xs bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 rounded-xl px-3 py-2 focus:outline-none focus:border-[#123465] cursor-pointer">
                    ...
                    <div className="flex flex-wrap gap-2 items-center">
                      <div className="relative">
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search student…"
                          className="pl-8 pr-3 py-2 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-[#123465] text-slate-700 dark:text-slate-200 w-44" />
